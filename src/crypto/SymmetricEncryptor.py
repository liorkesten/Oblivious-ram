...
class SymmetricEncryptor:
    ...
    def encrypt(self, data: bytes) -> SymmetricEncryptionObject:
        ...
        if num_of_padding_bytes > 0:
            ...
            print(f"Padding: {num_of_padding_bytes}")
    ...
    def decrypt(self, symmetric_crypto_object: SymmetricEncryptionObject) -> bytes:
        ...
        cipher = Cipher(algorithms.AES(self.key), modes.CBC(symmetric_crypto_object.iv))
        decryptor = cipher.decryptor()
        plain_text = decryptor.update(symmetric_crypto_object.cipher_text) + decryptor.finalize()
        plain_text = plain_text[:-symmetric_crypto_object.num_of_padding_bytes]

        print(f"Plain Text:{plain_text}. removed padding:{symmetric_crypto_object.num_of_padding_bytes}")
        return plain_text
